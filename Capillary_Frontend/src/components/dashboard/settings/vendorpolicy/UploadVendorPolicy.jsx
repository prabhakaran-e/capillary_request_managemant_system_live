...
import { deleteVendorPolicy, getVendorPolicy, saveVendorPolicy } from "../../../../api/service/adminServices";

const UploadVendorPolicy = () => {
    ...
    const fetchExistingFile = async () => {
        setIsLoading(true);
        try {
            const response = await getVendorPolicy();
            ...
            if (response.status === 200) {
                setExistingFile({
                    name: "vendor-policy-document.pdf",
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const uploadResponse = await uploadFilesVendor(selectedFile, "vendor-policy");
            ...
            if (uploadResponse) {
                ...
                const saveResponse = await saveVendorPolicy(fileUrl);

                if (saveResponse.status === 200) {
                    toast.success("Vendor Policy uploaded successfully!");
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const response = await deleteVendorPolicy();
       

            if (response.status === 200) {
                toast.success("Vendor Policy deleted successfully!");
                setExistingFile(null);
            }
        } catch (error) {
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-gray-900">
                                    Upload Vendor Policy
                                </h1>
                                <p className="text-gray-600 mt-1">
                                    Manage your vendor management policy document
                                </p>
                            </div>
                        </div>
                    </div>
...
export default UploadVendorPolicy;